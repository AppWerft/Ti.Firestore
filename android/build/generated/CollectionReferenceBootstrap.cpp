...
#define TAG "firebase.firestore"

using namespace v8;

static Persistent<Object> bindingCache;

static void CollectionReference_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "CollectionReference.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::CollectionReferenceBindings::lookupGeneratedInit(
...
static void CollectionReference_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, CollectionReference_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void CollectionReference_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::CollectionReferenceBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry CollectionReferenceBinding = {
	"firebase.firestore",
	CollectionReference_init,
	CollectionReference_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_firebase_firestore_CollectionReferenceBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("firebase.firestore", &CollectionReferenceBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::CollectionReferenceBindings::lookupGeneratedInit));
}
