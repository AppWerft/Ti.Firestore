...
void CollectionReferenceProxy::listen(const FunctionCallbackInfo<Value>& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(CollectionReferenceProxy::javaClass, "listen", "([Ljava/lang/Object;)V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'listen' with signature '([Ljava/lang/Object;)V'";
			LOGE(TAG, error);
				titanium::JSException::Error(isolate, error);
				return;
		}
