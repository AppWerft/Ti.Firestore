...
static void Tifirebase_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Tifirebase.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::TifirebaseBindings::lookupGeneratedInit(
...
static void Tifirebase_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Tifirebase_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Tifirebase_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::TifirebaseBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TifirebaseBinding = {
	"firebase.firestore",
	Tifirebase_init,
	Tifirebase_dispose
};
...
Java_firebase_firestore_TifirebaseBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("firebase.firestore", &TifirebaseBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::TifirebaseBindings::lookupGeneratedInit));
}
